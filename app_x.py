...
def main():
    ...
    optional.add_argument('-c', '--config', metavar="FILE",
                          default=Path(__file__).parent / 'cfg' / 'mot-TINY-coco.json',
    ...
    if args.mot:
        ...
        mot = fastmot.MOT_X(config.resize_to, **vars(config.mot_cfg), draw=draw)
        ...
        with Profiler('app') as prof:
            while not args.show or cv2.getWindowProperty('Video', 0) >= 0:
                ...
                if not(("http://" in args.input_uri) or ("rtsp://" in args.input_uri)):
                    ...
                    if frame is None:
                        print('There is no frames left, terminating...')
                        break
    ...
    # timing statistics
    if args.mot:
        avg_fps = mot.frame_count / prof.duration
        #logger.info('Frame count: %f, Duration: %f', mot.frame_count, prof.duration)
        logger.info('Average FPS: %f', avg_fps)
